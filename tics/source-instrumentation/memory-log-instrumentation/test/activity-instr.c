...
void ACCEL_singleSample(threeAxis_t_8* result){
    ...
    VMEM_WR(result->x) = (seed*17)%85;
    VMEM_WR(result->y) = (seed*17*17)%85;
    VMEM_WR(result->z) = (seed*17*17*17)%85;
...
void record_stats(stats_t *stats, class_t class)
{
    ...
    VMEM_WR_PI(stats->totalCount++);

    switch (class) {
        case CLASS_MOVING:
            VMEM_WR_PI(stats->movingCount++);
            break;

        case CLASS_STATIONARY:
            VMEM_WR_PI(stats->stationaryCount++);
            break;
    }
