  ...
  if (target_mcu)
    {
	  ...
	  snprintf (mcu_name, sizeof (mcu_name) - 1, "__MSP430i%s__", target_mcu + 7);
      else
	{
...
const char * const  ATTR_SEGSTACK = "segstack";
...
static inline bool
is_segstack_func (tree decl = current_function_decl)
{
  return has_attr (ATTR_SEGSTACK, decl);
}
		  ...
		  bool * no_add_attrs ATTRIBUTE_UNUSED)
{
  ...
  /* In some cases the front-end will place data before msp430_select_section
     ...
     section now.  */
  if (TREE_NAME_EQ (name, ATTR_PERSIST))
    set_decl_section_name (decl, ".persistent");
  else if (TREE_NAME_EQ (name, ATTR_NOINIT))
    set_decl_section_name (decl, ".noinit");
...
const struct attribute_spec msp430_attribute_table[] =
{
  ...
  { ATTR_SEGSTACK,    0, 0, true,  false, false, msp430_attr, false },
...
msp430_start_function (FILE *outfile, HOST_WIDE_INT hwi_local ATTRIBUTE_UNUSED)
{
  ...
  // IIKOE
  fprintf(outfile, "; framesize_morestack (iikoe): %d\n", SEGMENTED_STACK_FRAME_SIZE);
  fprintf(outfile, "; framesize_arg (iikoe): %d\n", crtl->args.size);
  // IIKOE

  // segmented stack code
  //rtx_insn *insn_arg = emit_insn (gen_push (GEN_INT (crtl->args.size)));
  //add_reg_note (insn_arg, REG_ARGS_SIZE, GEN_INT (2));
  //F (insn_arg);

  //insn = emit_insn (gen_push (GEN_INT (cfun->machine->framesize)));
  //add_reg_note (insn, REG_ARGS_SIZE, GEN_INT (2));
  //F (insn);
}
...
msp430_expand_prologue (void)
{
  ...
  emit_insn (gen_prologue_start_marker ());
    ...
    if (is_segstack_func ())
    {
        if (msp430x) {
            /*
             * If sp-(args.size+framesize)
             */
            // pushm.a #1, r12
            // mov.a #arg.size + framesize, r12
            // suba sp, r12 // what the new sp would be
            // cmp &__stack_limit,r12 //(dst - src) stack_limit is lowest
            // jl normal_execution
            // calla morestack
            // label: normal_execution

# if 0
            emit_move_insn (gen_rtx_REG(Pmode, 4), GEN_INT(crtl->args.size));
            emit_move_insn (gen_rtx_REG(Pmode, 5), GEN_INT(cfun->machine->framesize));
            emit_insn (gen_morestack_call ());
#endif
            HOST_WIDE_INT frame_total;
            HOST_WIDE_INT frame_args;

            frame_args = crtl->args.size;
            frame_total = frame_args + cfun->machine->framesize;
            emit_insn (gen_morestack_check_small (GEN_INT(frame_total), GEN_INT(frame_args)));
        }
    }

  // segmented stack code
  //rtx insn_arg = emit_insn (gen_pushm (gen_rtx_REG (Pmode, 5), GEN_INT (crtl->args.size)));
  //add_reg_note (insn_arg, REG_ARGS_SIZE, GEN_INT (2));
  //F (insn_arg);

  //rtx insn_arg = emit_insn (gen_morestack (GEN_INT (crtl->args.size), GEN_INT(2)));
  //add_reg_note (insn_arg, REG_ARGS_SIZE, GEN_INT (2));
  //F (insn_arg);
